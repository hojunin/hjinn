...
겟차에서의 의사결정은 기본적으로 수집된 **데이터**를 기반으로 합니다. 그렇기 때문에 사용자가 보기엔 달라지는 부분이 없어도, 클라이언트 사이드에서 명확한 데이터 수집 로직은 매우 중요한 작업 중 하나입니다. 이전까지는 특정 이벤트의 발생만을 매뉴얼하게 수집했습니다. 가령 _어떤 버튼을 클릭했다_, *어떤 화면을 봤다*와 같이 원하는 행동에 수집로직을 컴포넌트에 심어서 저희 데이터 서버로 보냈죠.
