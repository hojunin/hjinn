...
1.  mutex & semaphore
    OS시간에 배운 mutex & semaphore 개념에 착안해서 공유자원 접근에 대한 컨트롤타워 역할을 해줄 class를 singletone으로 만드는거에요. class는 최초 생성되고 세팅되기 전에 요청된 모든 API를 해당 클래스에서 관리하는 queue에 넣은 다음 모든 값이 세팅된 다음 쫙 호출해주는거죠.
...
저는 결국 두번째 방법을 선택했는데요, 단순히 코드가 쉽고 안전하다. 내가 없어도 충분히 이해할만한 코드다. 시점 자체가 명확하다 정도의 이유가 있었고, 첫번째 방법이 세팅되는 시점과 요청이 발생하는 시점의 간극이 벌어질수록 사용자 경험이 떨어질 것 같았고, 한번에 다량의 API가 호출될 가능성도 있었기 때문입니다.
