...
const ScreenTime = ({ data }: { data: SCREEN_TIME }) => {
  return (
    <div>
      <p>
        - 하루 평균{' '}
        <strong style={{ display: 'inline' }}>{`${data.time.average}`}</strong>
        이나 핸드폰을 붙잡고 살았어요
      </p>
      <p>
        - 그렇게 버린 시간이 한주간 총{' '}
        <strong style={{ display: 'inline' }}>{`${data.time.all}`}</strong>
        이에요.
      </p>
      {data.compare.status === 'same' ? (
        ...
        <p>
          - 지난 주 대비{' '}
          <strong style={{ display: 'inline' }}>{data.compare.percent}%</strong>
